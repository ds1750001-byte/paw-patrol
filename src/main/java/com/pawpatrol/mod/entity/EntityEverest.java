...
public class EntityEverest extends EntityCreature {

    private static final DataParameter<Boolean> IS_SITTING =
        EntityDataManager.createKey(EntityEverest.class, DataSerializers.BOOLEAN);

    public EntityEverest(World worldIn) {
        super(worldIn);
    ...
    protected void initEntityAI() {
        ...
        this.tasks.addTask(1, new EntityAISit(this) {
            @Override public boolean shouldExecute() { return EntityEverest.this.isSitting(); }
        });
        this.tasks.addTask(3, new EntityAIFollowEverestTask(this));
    ...
    public boolean processInteract(EntityPlayer player, EnumHand hand) {
        if (player.isSneaking()) {
            if (!this.world.isRemote) {
                ...
                if (nowSitting) {
                    player.sendMessage(new TextComponentString("§3Эверест ждёт на месте."));
                } else {
                    player.sendMessage(new TextComponentString("§3Эверест следует за тобой!"));
                }
            }
            return true;
        }
