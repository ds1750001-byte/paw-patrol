...
public class EntityMarshall extends EntityCreature {

    private static final DataParameter<Boolean> IS_SITTING =
        EntityDataManager.createKey(EntityMarshall.class, DataSerializers.BOOLEAN);

    public EntityMarshall(World worldIn) {
        super(worldIn);
    ...
    protected void initEntityAI() {
        ...
        this.tasks.addTask(1, new EntityAISit(this) {
            @Override public boolean shouldExecute() { return EntityMarshall.this.isSitting(); }
        });
        this.tasks.addTask(3, new EntityAIFollowMarshallTask(this));
    ...
    public boolean processInteract(EntityPlayer player, EnumHand hand) {
        if (player.isSneaking()) {
            if (!this.world.isRemote) {
                ...
                if (nowSitting) {
                    player.sendMessage(new TextComponentString("§cМаршалл ждёт на месте."));
                } else {
                    player.sendMessage(new TextComponentString("§cМаршалл следует за тобой!"));
                }
            }
            return true;
        }
