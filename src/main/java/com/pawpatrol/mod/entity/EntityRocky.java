...
public class EntityRocky extends EntityCreature {

    private static final DataParameter<Boolean> IS_SITTING =
        EntityDataManager.createKey(EntityRocky.class, DataSerializers.BOOLEAN);

    public EntityRocky(World worldIn) {
        super(worldIn);
    ...
    protected void initEntityAI() {
        ...
        this.tasks.addTask(1, new EntityAISit(this) {
            @Override public boolean shouldExecute() { return EntityRocky.this.isSitting(); }
        });
        this.tasks.addTask(3, new EntityAIFollowRockyTask(this));
    ...
    public boolean processInteract(EntityPlayer player, EnumHand hand) {
        if (player.isSneaking()) {
            if (!this.world.isRemote) {
                ...
                if (nowSitting) {
                    player.sendMessage(new TextComponentString("§2Роки ждёт на месте."));
                } else {
                    player.sendMessage(new TextComponentString("§2Роки следует за тобой!"));
                }
            }
            return true;
        }
