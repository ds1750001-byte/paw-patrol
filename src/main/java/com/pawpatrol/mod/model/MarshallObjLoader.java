...
public class MarshallObjLoader {

    private static float[] bodyMesh = null;
    private static float[] packMesh = null;
    private static boolean loaded = false;

    private static final int BODY_VERT_COUNT = 8160;
    private static final int PACK_VERT_COUNT = 1698;

    public static void ensureLoaded() {
        if (loaded) return;
        try {
            bodyMesh = loadBinaryMesh("assets/pawpatrol/models/entity/marshall_body.bin", BODY_VERT_COUNT);
            packMesh = loadBinaryMesh("assets/pawpatrol/models/entity/marshall_pack.bin", PACK_VERT_COUNT);
            loaded = true;
            PawPatrolMod.logger.info("Marshall mesh loaded!");
        } catch (Exception e) {
            PawPatrolMod.logger.error("Failed to load Marshall mesh!", e);
        }
    }

    private static float[] loadBinaryMesh(String path, int vertCount) throws IOException {
        InputStream is = MarshallObjLoader.class.getClassLoader().getResourceAsStream(path);
