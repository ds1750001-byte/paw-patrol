...
/**
 * Загружает и рендерит бинарные данные меша Рабла.
 */
@SideOnly(Side.CLIENT)
public class RubbleObjLoader {

    private static float[] bodyMesh = null;
    private static float[] packMesh = null;
    private static boolean loaded = false;

    private static final int BODY_VERT_COUNT = 9492;
    private static final int PACK_VERT_COUNT = 1542;

    public static void ensureLoaded() {
        if (loaded) return;
        try {
            bodyMesh = loadBinaryMesh("assets/pawpatrol/models/entity/rubble_body.bin", BODY_VERT_COUNT);
            packMesh = loadBinaryMesh("assets/pawpatrol/models/entity/rubble_pack.bin", PACK_VERT_COUNT);
            loaded = true;
            PawPatrolMod.logger.info("Rubble mesh loaded!");
        } catch (Exception e) {
            PawPatrolMod.logger.error("Failed to load Rubble mesh!", e);
        }
    }

    private static float[] loadBinaryMesh(String path, int vertCount) throws IOException {
        InputStream is = RubbleObjLoader.class.getClassLoader().getResourceAsStream(path);
    ...
    private static void renderMesh(float[] mesh, int vertCount) {
        ...
        for (int i = 0; i < vertCount; i++) {
            int base = i * stride;
            buf.pos(mesh[base], mesh[base+1], mesh[base+2])
               .tex(mesh[base+3], mesh[base+4]).endVertex();
        }
        tess.draw();
    }
}
