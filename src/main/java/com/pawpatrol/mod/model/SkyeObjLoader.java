...
public class SkyeObjLoader {

    private static float[] bodyMesh = null;
    private static float[] packMesh = null;
    private static boolean loaded = false;

    private static final int BODY_VERT_COUNT = 8970;
    private static final int PACK_VERT_COUNT = 642;

    public static void ensureLoaded() {
        if (loaded) return;
        try {
            bodyMesh = loadBinaryMesh("assets/pawpatrol/models/entity/skye_body.bin", BODY_VERT_COUNT);
            packMesh = loadBinaryMesh("assets/pawpatrol/models/entity/skye_pack.bin", PACK_VERT_COUNT);
            loaded = true;
            PawPatrolMod.logger.info("Skye mesh loaded!");
        } catch (Exception e) {
            PawPatrolMod.logger.error("Failed to load Skye mesh!", e);
        }
    }

    private static float[] loadBinaryMesh(String path, int vertCount) throws IOException {
        InputStream is = SkyeObjLoader.class.getClassLoader().getResourceAsStream(path);
